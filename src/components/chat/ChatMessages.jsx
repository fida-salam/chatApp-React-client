...
// import TypingIndicator from "./TypingIndicator";
// import EmojiReactions from './EmojiReactions'; // next section
...
import { useAuthStore } from "../../store/authStore";
import { formatDistanceToNow } from "date-fns";
import { Loader2, File, Download, Check, CheckCheck, Pencil, Trash2, X } from "lucide-react";
import TypingIndicator from "./TypingIndicator";

function ContextMenu({ x, y, isOwn, onEdit, onDelete, onClose }) {
  ...
  return (
    <div
      ...
      className="z-50 bg-card border rounded-lg shadow-lg py-1 min-w-[140px] text-sm"
    >
      {isOwn && (
        <>
          <button
      ...
      {!isOwn && (
        <button
          onClick={onClose}
          className="w-full flex items-center gap-2 px-3 py-2 hover:bg-accent transition-colors text-muted-foreground"
        >
          <X className="w-4 h-4" />
          Close
        </button>
