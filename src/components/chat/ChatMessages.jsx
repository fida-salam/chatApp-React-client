    ...
    <div className="flex-1 overflow-y-auto p-2 sm:p-4 space-y-3 sm:space-y-4">
      {!hasMessages ? (
        ...
        roomMessages.map((message) => {
          const isOwn =
            message && message.sender?._id?.toString() === currentUserId?.toString();
          const rawDate = message && message.createdAt ? new Date(message.createdAt) : null;
const time = rawDate && !isNaN(rawDate)
  ? formatDistanceToNow(rawDate, { addSuffix: true })
  : "just now";
          const isRead = (message && message.readBy?.length ? message.readBy.length : 0) > 1;
          const isEditing =
            editingMessage &&
            message &&
            ...
            >
              ...
              >
                ...
                <div className="flex flex-col">
                  {/* Sender name */}
                  {!isOwn && message.sender && (
                    <p className="text-xs sm:text-xs text-muted-foreground mb-1 ml-1 sm:ml-2">
                  ...
                  {/* Emoji reactions - with bottom padding on mobile for picker */}
